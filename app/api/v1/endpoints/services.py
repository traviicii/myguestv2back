...
def list_services(
    ...
    stmt = select(Service).where(Service.owner_user_id == current_user.id)
    if active == "true":
        stmt = stmt.where(Service.is_active.is_(True))
    elif active == "false":
        stmt = stmt.where(Service.is_active.is_(False))

    items = list(db.scalars(stmt.order_by(Service.sort_order.asc(), Service.name.asc())).all())
    return ServiceListResponse(items=[ServiceRead.model_validate(item) for item in items])
