...
- New uploads target S3-compatible private object storage plus CDN.
- Access model is private by default with short-lived signed delivery.
- Legacy Firebase image references stay readable during migration.
- Variants are generated for `thumb`, `medium`, and `original`.
- Default upload limit is `10 MB` per image.
- Allowed formats are `jpeg`, `png`, `heic`, and `webp`.

## Current State

- `formula_images` already supports `storage_provider`, `public_url`, and `object_key`.
- The mobile app currently sends raw image references, including `file://` device paths and remote URLs.
